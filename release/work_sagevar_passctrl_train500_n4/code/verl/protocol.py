...
class DataProto:
    ...
    def custom_repeat_by_counts(self, repeat_counts, interleave=True):
        """Repeat each row by repeat_counts[i] times.

        Args:
            repeat_counts (List[int]): per-row repeat counts; length must equal current batch_size.
            interleave (bool): when True (default), row i appears repeat_counts[i] times consecutively;
                when False, rows are tiled in row-block order.
        Returns:
            DataProto with new batch_size = sum(repeat_counts).
        """
        repeat_counts = list(repeat_counts)
        if self.batch is not None:
            cur_bsz = self.batch.batch_size[0]
        elif self.non_tensor_batch:
            cur_bsz = len(next(iter(self.non_tensor_batch.values())))
        else:
            cur_bsz = 0
        assert len(repeat_counts) == cur_bsz, (
            f"repeat_counts length {len(repeat_counts)} must equal batch_size {cur_bsz}"
        )

        if interleave:
            indices = [i for i, c in enumerate(repeat_counts) for _ in range(c)]
        else:
            max_c = max(repeat_counts) if repeat_counts else 0
            indices = [i for k in range(max_c) for i, c in enumerate(repeat_counts) if k < c]

        if self.batch is not None:
            idx_t = torch.as_tensor(indices, dtype=torch.long)
            repeated_tensors = {key: tensor[idx_t] for key, tensor in self.batch.items()}
            repeated_batch = TensorDict(source=repeated_tensors, batch_size=(len(indices),))
        else:
            repeated_batch = None

        idx_np = np.asarray(indices, dtype=np.int64)
        repeated_non_tensor_batch = {
            key: val[idx_np] for key, val in self.non_tensor_batch.items()
        }
