...
class vLLMMultiTurnViaChatRollout(BaseRollout):
    ...
    def get_n_tokens(self,prompt,add_generation_prompt=False):
        return len(self.tokenizer.apply_chat_template(prompt,tokenize=True,add_generation_prompt=add_generation_prompt,enable_thinking=False))
    
    def tokenize_with_assistant_mask(self,messages):
        """Build assistant mask by scanning for role marker tokens in the tokenized sequence.
        
        Instead of incremental apply_chat_template (which fails with Qwen3's non-monotonic
        <think> block handling), we tokenize ONCE and find assistant boundaries by locating
        <|im_start|>assistant pattern in the token sequence.
        """
        _result = self.tokenizer.apply_chat_template(
            messages, tokenize=True, add_generation_prompt=False, enable_thinking=False)
        if isinstance(_result, list):
            tokenized_messages = _result
        else:
            ids = _result['input_ids']
            tokenized_messages = ids[0] if isinstance(ids, list) and len(ids) > 0 and isinstance(ids[0], list) else list(ids)
        
        # Token IDs for Qwen3 chat format markers
        im_start_id = self.tokenizer.convert_tokens_to_ids('<|im_start|>')  # 151644
        assistant_role_id = self.tokenizer.encode('assistant', add_special_tokens=False)[0]  # 77091
        
        # Find all <|im_start|> positions and determine message boundaries
        im_start_positions = [i for i, tok in enumerate(tokenized_messages) if tok == im_start_id]
        
        # Build mask: for each segment, check if it's an assistant message
        assistant_mask = torch.zeros(len(tokenized_messages), dtype=torch.bool)
        
        for seg_idx, start_pos in enumerate(im_start_positions):
            # Determine end of this segment (start of next segment or end of sequence)
            if seg_idx + 1 < len(im_start_positions):
                end_pos = im_start_positions[seg_idx + 1]
            else:
                end_pos = len(tokenized_messages)
            
            # Check if this segment is assistant: <|im_start|> followed by assistant token
            if start_pos + 1 < len(tokenized_messages) and tokenized_messages[start_pos + 1] == assistant_role_id:
                assistant_mask[start_pos:end_pos] = True
    ...
    def generate_sequences(self, prompts: DataProto,**kwargs)->DataProto:#, environment, **kwargs) -> DataProto: #see verl/single_controller/base/decorator l.54, we can't send these classes as usual.
        ...
        # Detailed sampling log setup
        _detailed_log = os.environ.get("RLVER_DETAILED_LOG", "") == "true"
        _log_records = [] if _detailed_log else None
        ...
        while True:
            ...
            for output in outputs:
                for sample_id in range(len(output.outputs)):
                    response.append(output.outputs[sample_id].token_ids)
            # print(f"response: {response}")  # removed: too verbose
            if not response:
            ...
            env_response_batched = [None] * len(messagess_todo)
            # batch request — parallel DPV3 API calls (safe: preserves order, fallback to serial)
            _max_workers = int(os.environ.get("PLAYER_MAX_WORKERS", "16"))
            def _player_call(idx, i_sim, msgs):
                assert msgs[-1]["role"] == "assistant"
                return idx, player_simulators[i_sim].reply(msgs[-1]["content"])
            try:
                if _max_workers <= 0:
                    raise ValueError("serial mode")
                from concurrent.futures import ThreadPoolExecutor, as_completed
                with ThreadPoolExecutor(max_workers=min(len(messagess_todo), _max_workers)) as pool:
                    futs = [pool.submit(_player_call, j, todo[j], messagess_todo[j].copy()) for j in range(len(messagess_todo))]
                    for fut in as_completed(futs):
                        _fidx, resp = fut.result()
                        env_response_batched[_fidx] = resp
            except Exception as e:
                print(f"[WARN] parallel player failed ({e}), falling back to serial")
                env_response_batched = []
                for i in range(len(messagess_todo)):
                    msg = messagess_todo[i].copy()
                    assert msg[-1]["role"]=="assistant"
                    env_response_batched.append(player_simulators[todo[i]].reply(msg[-1]["content"]))
        ...
        for i in range(len(player_simulators)):
            ...
            # Collect detailed per-conversation log
            if _detailed_log:
                _conv_msgs = [{"role": m["role"], "content": m["content"][:300]} for m in messagess[i]]
                _log_records.append({
                    "conv_id": i,
                    "profile_id": getattr(player_simulators[i], 'role', {}).get('id', ''),
                    "emo_init": 30,
                    "emo_final": player_simulators[i].emo_point,
                    "emo_state": player_simulators[i].emo_state,
                    "n_turns": len([m for m in messagess[i] if m["role"] == "assistant"]),
                    "turn_log": player_simulators[i].turn_log,
                    "messages": _conv_msgs,
                })

        # Write detailed per-conversation sampling log
        if _detailed_log and _log_records:
            import json as _json
            _ckpt_dir = os.environ.get("RLVER_RUN_CKPT_DIR", "")
            if _ckpt_dir:
                _log_dir = os.path.join(_ckpt_dir, "sampling_logs")
                os.makedirs(_log_dir, exist_ok=True)
                _log_file = os.path.join(_log_dir, "sampling_detail.jsonl")
