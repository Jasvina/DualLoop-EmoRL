...
class PlayerSimulator:
    ...
    def __init__(self, save_dir, external_profile=None):
        ...
        self.data = []
        self._external_profile = external_profile
        ...
        self.eq_role_file = os.path.join(_base_dir, "data/train_profile.jsonl")

        if external_profile is not None:
            # Self-play mode: use externally provided profile
            self.role = {
                "id": external_profile["id"],
                "emo_point": self.emo_point,
                "emo_state": self.emo_state,
                "target": "eq",
                "player": external_profile["player"],
                "scene": external_profile["scene"],
                "character": external_profile.get("main_cha", "被动型"),
                "topic": external_profile.get("topic", external_profile.get("task", "")),
                "history": [],
            }
            self.chat_player(self.role)
        else:
            # Original mode: sample from train_profile.jsonl
        ...
        while retries < max_retries:
            try:
                ...
                if retries >= max_retries:
                    print(f"[planning_reply] Max retries ({max_retries}) exceeded, using change=0")
                    break
                time.sleep(3)
...
"""
    ...
    def save_player_data(self):
        try:
            ...
            filename = f"simulator_results_{timestamp}_pid{os.getpid()}.jsonl"
            ...
            print(f"[save_player_data] write skipped: {_e}")

    def clone(self):
        new_simulator = PlayerSimulator(self.save_dir, external_profile=self._external_profile)
