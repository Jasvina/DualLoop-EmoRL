...
class DataParallelPPOCritic(BasePPOCritic):
    ...
    def compute_values(self, data: DataProto) -> torch.Tensor:
        ...
        if 'generation_mask' in data.batch:
        ...
        if 'generation_mask' in data.batch:
            value_mask = (attention_mask * data.batch['generation_mask'])[:, -response_length - 1:-1]
        else:
        ...
        for epoch in range(self.config.ppo_epochs):
            for batch_idx, data in enumerate(dataloader):
                ...
                for data in micro_batches:
                    ...
                    if 'generation_mask' in data:
                        eos_mask = (attention_mask * data['generation_mask'])[:, -response_length - 1:-1]
                    else:
