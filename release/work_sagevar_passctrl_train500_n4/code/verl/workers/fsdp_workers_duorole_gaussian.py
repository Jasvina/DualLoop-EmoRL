...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_rollout(self):
        ...
        if self.config.rollout.name == 'hf':
            ...
            #                                    model_hf_config=self.actor_model_config)
                ...
                from verl.workers.rollout.vllm_rollout.vllm_rollout_spmd_duorole_gaussian import vLLMMultiTurnViaChatRollout_think as vLLMMultiTurnViaChatRolloutDuoRoleSR
                if vllm_mode == 'customized':
                    rollout = vLLMMultiTurnViaChatRolloutDuoRoleSR(actor_module=self.actor_module_fsdp,
                                    config=self.config.rollout,
                                    tokenizer=self.tokenizer,
                                    model_hf_config=self.actor_model_config)
                elif vllm_mode == 'spmd':
                    rollout = vLLMMultiTurnViaChatRolloutDuoRoleSR(model_path=local_path,
                                    config=self.config.rollout,
                                    tokenizer=self.tokenizer,
                                    model_hf_config=self.actor_model_config,
                                    device_mesh=rollout_device_mesh)
    ...
    def init_model(self):
        from verl.workers.actor.dp_actor_duorole_gaussian import DataParallelPPOActor
