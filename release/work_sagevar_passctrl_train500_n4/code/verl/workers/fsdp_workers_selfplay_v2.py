...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_rollout(self):
        ...
        if self.config.rollout.name == 'hf':
            ...
            #                                    model_hf_config=self.actor_model_config)
                ...
                if vllm_mode == 'customized':
                    if self.config.thinking:
                        rollout = vLLMMultiTurnViaChatRollout_think(actor_module=self.actor_module_fsdp,
                                        config=self.config.rollout,
                                        tokenizer=self.tokenizer,
                                        model_hf_config=self.actor_model_config)
                    else:
    ...
    def init_model(self):
        from verl.workers.actor.dp_actor_selfplay_v2 import DataParallelPPOActor
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def generate_scenes(self, prompts: DataProto):
        """Generate scenes using vLLM inference engine directly (bypass multi-turn rollout)."""
        from verl.workers.rollout.vllm_rollout.vllm_rollout_spmd import pad_2d_list_to_length, get_eos_mask, _pre_process_inputs
        prompts = prompts.to("cuda")
        assert self._is_rollout
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)
        ...
        with self.rollout_sharding_manager:
            ...
            _sp = SamplingParams(temperature=1.0, top_p=1.0, max_tokens=512, n=1)
            outputs = self.rollout.inference_engine.generate(
                prompts=idx_list, sampling_params=_sp, use_tqdm=False)
        response = []
        for output in outputs:
            for sample in output.outputs:
                response.append(sample.token_ids)
        response = pad_2d_list_to_length(response, pad_token_id, max_length=512).to(idx.device)
        seq = torch.cat([idx, response], dim=-1)
        response_length = response.size(1)
        delta_pos = torch.arange(1, response_length + 1, device=position_ids.device).unsqueeze(0).repeat(batch_size, 1)
        response_position_ids = position_ids[:, -1:] + delta_pos
        position_ids = torch.cat([position_ids, response_position_ids], dim=-1)
        response_attention_mask = get_eos_mask(response_id=response, eos_token=eos_token_id, dtype=attention_mask.dtype)
        attention_mask = torch.cat((attention_mask, response_attention_mask), dim=-1)
        from tensordict import TensorDict as _TD
        batch = _TD({"prompts": idx, "responses": response, "input_ids": seq,
                     "attention_mask": attention_mask, "position_ids": position_ids}, batch_size=batch_size)
