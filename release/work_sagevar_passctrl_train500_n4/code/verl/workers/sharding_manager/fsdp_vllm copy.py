...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def __enter__(self):
        ...
        # Free PyTorch cache before wake_up to maximize available GPU memory
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        if vllm_version in ('0.4.2', '0.5.4', '0.6.3'):
            ...
            # level=0: free KV cache only, keep model weights on GPU
            # Prevents OOM when other processes claim freed GPU memory
            # (level=1 would fully offload weights, risking OOM on wake_up)
    ...
    def preprocess_data(self, data: DataProto) -> DataProto:
        ...
        tp_world_size = vllm_ps.get_tensor_model_parallel_world_size()
        if tp_world_size <= 1:
            # No need to allgather when TP=1
            pass
        elif vllm_version in ('0.3.1', '0.4.2', '0.5.4', '0.6.3'):
            data.batch = allgather_dict_tensors(data.batch.contiguous(),
                                                size=tp_world_size,
                                                group=vllm_ps.get_tensor_model_parallel_group(),
                                                dim=0)
        else:
            # vLLM >= 0.7: get_tensor_model_parallel_group() -> get_tp_group()
            tp_group = vllm_ps.get_tp_group()
            group = getattr(tp_group, 'device_group', tp_group)
            data.batch = allgather_dict_tensors(data.batch.contiguous(),
                                                size=tp_world_size,
                                                group=group,
                                                dim=0)

        if data.non_tensor_batch is not None and len(data.non_tensor_batch) > 0 and tp_world_size > 1:
            ...
            for key, val in data.non_tensor_batch.items():
                ...
                if vllm_version in ('0.3.1', '0.4.2', '0.5.4', '0.6.3'):
                    ...
                    tp_group = vllm_ps.get_tp_group()
                    group = getattr(tp_group, 'device_group', tp_group)
                    torch.distributed.all_gather_object(gathered, val, group=group)
