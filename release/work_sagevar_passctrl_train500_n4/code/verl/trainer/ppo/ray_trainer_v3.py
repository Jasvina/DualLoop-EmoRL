            ...
            for batch_dict in tqdm(self.train_dataloader, desc=f"Epoch {epoch + 1} / {self.config.trainer.total_epochs}", initial=tqdm_initial):
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        # [V3 FIX] Ensure GRPO doesn't mix user and assistant rows.
                        # Give user rows a different uid so compute_grpo_outcome_advantage
                        # naturally separates them into different groups.
                        _v3_dual = os.environ.get("V3_DUAL_ROLE", "") == "true"
                        _turn_roles_pre = batch.non_tensor_batch.get('turn_role', None) if _v3_dual else None
                        
                        if _turn_roles_pre is not None:
                            _roles_pre = np.array([str(r) for r in _turn_roles_pre])
                            _uids = batch.non_tensor_batch['uid']
                            # Append "_user" to user rows' uid so GRPO won't mix them with assistant rows
                            for _i in range(len(_uids)):
                                if _roles_pre[_i] == "user":
                                    _uids[_i] = str(_uids[_i]) + "_user"
                            batch.non_tensor_batch['uid'] = _uids
                            
                            # [V3 DEBUG] Print uid distribution to verify separation
                            _asst_uids = [_uids[i] for i in range(len(_uids)) if _roles_pre[i] == "assistant"]
                            _user_uids = [_uids[i] for i in range(len(_uids)) if _roles_pre[i] == "user"]
                            from collections import Counter
                            _asst_groups = Counter(_asst_uids)
                            _user_groups = Counter(_user_uids)
                            print(f"[V3 DEBUG] Assistant uids (first 5): {list(_asst_groups.keys())[:5]}")
                            print(f"[V3 DEBUG] User uids (first 5): {list(_user_groups.keys())[:5]}")
                            print(f"[V3 DEBUG] Assistant group sizes: {list(_asst_groups.values())[:5]}")
                            print(f"[V3 DEBUG] User group sizes: {list(_user_groups.values())[:5]}")
                            print(f"[V3 DEBUG] Total: {len(_asst_uids)} assistant rows, {len(_user_uids)} user rows")
                        
                        # Now compute_advantage: GRPO will group by uid.
                        # Assistant rows group together (same scene uid).
                        # User rows group together (same scene uid + "_user").
                        ...
                        # [V3] After compute_advantage, override user rows' advantages
                        # with realism-based scoring (scaled by λ).
                        # compute_advantage used token_level_rewards (emotion-based) for all rows,
                        # but user rows should use realism_score instead.
                        if _turn_roles_pre is not None:
                            _v3_lambda = float(os.environ.get("V3_USER_LAMBDA", "0.1"))
                            _user_idx = np.where(_roles_pre == "user")[0]
                            
                            if len(_user_idx) > 0:
                                _realism = batch.non_tensor_batch.get('realism_score', None)
                                if _realism is not None:
                                    _realism_arr = np.array([float(_realism[i]) for i in _user_idx])
                                    _realism_t = torch.tensor(_realism_arr, dtype=torch.float32)
                                    # GRPO z-score within user rows only
                                    _n = self.config.actor_rollout_ref.rollout.n
                                    _n_groups = max(1, len(_realism_t) // _n)
                                    _user_adv = torch.zeros_like(_realism_t)
                                    for _gi in range(_n_groups):
                                        _s = _gi * _n
                                        _e = min(_s + _n, len(_realism_t))
                                        _grp = _realism_t[_s:_e]
                                        _m = _grp.mean()
                                        _st = _grp.std() + 1e-6
                                        _user_adv[_s:_e] = (_grp - _m) / _st
                                    
                                    # Override user rows with realism advantage × λ
                                    _advantages = batch.batch['advantages']
                                    _device = _advantages.device
                                    for _ii, _batch_idx in enumerate(_user_idx):
                                        if _batch_idx < _advantages.shape[0]:
                                            _advantages[_batch_idx, :] = (_user_adv[_ii] * _v3_lambda).to(_device)
                                    batch.batch['advantages'] = _advantages
                                    
                                    print(f"[V3] GRPO separated: {len(np.where(_roles_pre == 'assistant')[0])} "
                                          f"assistant rows, {len(_user_idx)} user rows (λ={_v3_lambda}), "
                                          f"mean_realism={_realism_t.mean():.3f}")
                                else:
                                    # No realism: zero out user rows
                                    _advantages = batch.batch['advantages']
                                    for _batch_idx in _user_idx:
                                        if _batch_idx < _advantages.shape[0]:
                                            _advantages[_batch_idx, :] = 0.0
                                    batch.batch['advantages'] = _advantages
                                    print("[V3] WARNING: no realism_score, user rows zeroed")
