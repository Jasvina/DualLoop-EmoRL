...
def build_duorole_rewards(data: DataProto, success_threshold: float, user_lambda: float):
    ...
    for i in range(len(data)):
        assistant_scalar_scores[i] = float(emo_points[i]) / 100.0
        user_scalar_scores[i] = scene_to_user_reward.get(scene_uids[i], -0.5)
    ...
    data.non_tensor_batch['duorole_sr'] = np.array([scene_to_sr.get(scene_uids[i], 0.0) for i in range(len(data))],
                                                   dtype=object)
    data.non_tensor_batch['duorole_user_reward'] = np.array([scene_to_user_reward.get(scene_uids[i], -0.5)
                                                             for i in range(len(data))], dtype=object)
    ...
    metrics = {
        ...
        'duorole/user_reward_mean': float(np.mean(list(scene_to_user_reward.values()))) if scene_to_user_reward else 0.0,
...
def compute_duorole_advantage(data: DataProto, user_lambda: float):
    ...
    scene_order = sorted(set(scene_uids.tolist()))
    scene_rewards = np.array([
        float(user_rewards[np.where(scene_uids == scene_uid)[0][0]]) for scene_uid in scene_order
    ], dtype=np.float32)
    scene_user_adv = _zscore(scene_rewards)
    for scene_uid, adv in zip(scene_order, scene_user_adv):
        row_idx_np = np.where(scene_uids == scene_uid)[0]
        row_idx = torch.tensor(row_idx_np, dtype=torch.long, device=advantages.device)
