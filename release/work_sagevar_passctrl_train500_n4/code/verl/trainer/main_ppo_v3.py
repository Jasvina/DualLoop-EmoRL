...
def main_task(config, compute_score=None):
    ...
    if config.actor_rollout_ref.actor.strategy == 'fsdp':
        ...
        # [V3] Use V3 workers (V3 rollout + V3 actor)
        from verl.workers.fsdp_workers_v3 import ActorRolloutRefWorker, CriticWorker
    ...
    from verl.trainer.ppo.ray_trainer_v3 import ResourcePoolManager, Role
    ...
    if use_environment:
        if not config.actor_rollout_ref.actor.use_loss_generation_mask:
            print("[V3] use_loss_generation_mask=False; V3 dual-role uses row-separation, generation_mask not required.")
        ...
        elif environment_name == "sage_environment":
            from verl.environments import SAGEEnvironment
            environment = SAGEEnvironment(config=config, tokenizer=tokenizer)
        else:
            raise NotImplementedError(f"environment {environment_name} not implemented")

        resource_pool_manager = ResourcePoolManager(resource_pool_spec=resource_pool_spec, mapping=mapping)
        # [V3] Always use V3 trainer (dual-role self-play)
        from verl.trainer.ppo.ray_trainer_v3 import RayPPOTrainer
